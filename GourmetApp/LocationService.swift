...
public class LocationService: NSObject, CLLocationManagerDelegate {
    ...
    public var locationServiceDisabledAlert: UIAlertController {
        get {
            let alert = UIAlertController(
                title: "位置情報が取得できません",
                message: "設定からプライバシー → 位置情報画面を開いてGoumetSearchの位置情報の許可を「このAppの使用中のみ許可」と設定してください。",
    ...
    public var locationServiceRestrictedAlert: UIAlertController {
        get {
            let alert = UIAlertController(
                title: "位置情報が取得できません",
                message: "設定から一般 → 機能制限画面を開いてGoumetSearchのいち情報の許可を「このAppの使用中のみ許可」と設定してください。",
                preferredStyle: .Alert)
    ...
    public func locationManager(manager: CLLocationManager!, didUpdateLocations locations: [AnyObject]!) {
        ...
        if let location = locations.last as? CLLocation {
            // 位置情報を載せてNotificationを送信する
            nsnc.postNotificationName(LSDidUpdateLocationNotification, object: self, userInfo: ["location": location])
        }
    }
